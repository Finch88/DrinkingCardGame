...
public class RoundFour extends Round{
	...
	public void setScreen(Graphics g, ArrayList<Image> images, ArrayList<Button> buttons, ArrayList<TextField> textFields){
		...
		try{
			buttons.get(0).setLabel("HEARTS");		
			buttons.get(0).setSize(150,50);
			buttons.get(0).setLocation(20,425);
		
			buttons.get(1).setFont(smallLabelFont);
			buttons.get(1).setLabel("DIAMONDS");
			buttons.get(1).setSize(150,50);
			buttons.get(1).setLocation(180,425);

			buttons.get(2).setLabel("SPADES");		
			buttons.get(2).setSize(150,50);
			buttons.get(2).setLocation(20,500);
		
			buttons.get(3).setLabel("CLUBS");
			buttons.get(3).setSize(150,50);
			buttons.get(3).setLocation(180,500);

			// Set location of next button for after playRound()
			buttons.get(4).setLocation(100,425);
		}
	...
	public void playRound(Player player, String guess, Deck deck){
		...
		else if((dealtCard.getSuit()=="S"&&guess=="SPADES")||
				(dealtCard.getSuit()=="C"&&guess=="CLUBS")||
				(dealtCard.getSuit()=="H"&&guess=="HEARTS")||
				(dealtCard.getSuit()=="D"&&guess=="DIAMONDS"))
		{	
		...
		player.dealtCards.add(3,dealtCard);
	}
}
